...
def main():
    ...
    correct = 0
    incorrect = 0
    StateCapitalPair = list(stateCapitals.items())
    random.shuffle(StateCapitalPair)

    for state, capital in StateCapitalPair:
        question = input(f"What is the state capital of {state}?")
        if question.strip().lower() == capital.lower():
        #if question == capital:
            print("Yes!")
            correct +=1
        else:
            print("Not quite, better luck next time! The correct answer is", capital)
            incorrect += 1
